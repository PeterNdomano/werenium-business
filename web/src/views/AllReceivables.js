...
import OneReceivable from '../ones/OneReceivable';
import $ from 'jquery';

export default class AllReceivables extends Component{

  /*
  This was duplicated from AllIncomes,
  some variable names are left for compatibility only.
  */
  ...
  getIncomes = async (show = false) => {
    let incomes = await this.props.business.getAccountsReceivable(); //key is here
  ...
  setIncomesValue = () => {
    ...
    this.incomes.forEach((item)=> {
      total += Number(item.amountDue);
      collected += Number(item.amountPaid);
    });
    uncollected = total - collected;
    this.incomesValue = total;
    if(this.props.showTotal === true){
      document.getElementById('_totalReceivablesDisplay').innerHTML = numberFormat(total);
      document.getElementById('_totalCollectedDisplay').innerHTML = numberFormat(collected);
      document.getElementById('_totalUncollectedDisplay').innerHTML = numberFormat(uncollected);
    }
  }
        ...
        this.incomesShow = this.incomesFiltered.map((item, index) => {
          if(limit > 0){
            if(index >= startIndex){
              --limit;
              return (
                <OneReceivable reload={this.props.reload} openViewer={this.props.openViewer} business={this.props.business} getIncomes={this.getIncomes} showDialogView={this.props.showDialogView} openViewer={this.props.openViewer} business={this.props.business} showDialog={this.props.showDialog} key={item.id} item={item}/>
              );
            }
          }
          else{
